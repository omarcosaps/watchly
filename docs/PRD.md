...
No telefone, a faixa de selects some. A pessoa filtra pelo botão do topo, que abre um painel na base da tela com os mesmos controles e a contagem. O painel fecha pelo overlay, Escape ou o botão fechar, e fica acima da tab bar. O botão mostra um indicador discreto quando há filtro além do tipo (gênero, ano, provedor ou ordenação). A partir de 640px a faixa permanece abaixo do hero, sem botão no header. Os filtros aplicam na hora pela URL, como hoje: Tipo troca de tela com o mesmo gesto da nav; gênero, ano, provedor e ordenação não re-animam a página.
